  ...
  const handleSkip = async (doseId: string) => {
    try {
      const res = await fetch("/api/v1/doses", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ doseId, action: "SKIPPED", actor: "Patient (Self)", reason: "Skipped by patient" }),
      });
      const json = await res.json();
      if (json.success) {
        loadDoses();
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Track taken, missed, and upcoming doses. Local device alerts ensure adherence without internet.
          </p>
        </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">
                      Generic: {dose.genericName || "Verified Bangladesh formulation"}
                    </p>
